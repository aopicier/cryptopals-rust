...
fn mitm_handle_client<T: Communicate>(client_stream: T, server_stream: T) -> Result<()> {
    ...
    loop {
        match mitm.receive_client()? {
            Some(message) => {
                compare(mitm.decrypt_client(message.clone())?, Some(b"This is a test".to_vec()))?;
        ...
        match mitm.receive_server()? {
            Some(message) => {
                compare(mitm.decrypt_server(message.clone())?, Some(b"This is a test".to_vec()))?;
...
fn matasano5_34_echo() -> Result<()> {
    ...
    let stream = 
        ...
        .chain_err(|| "client failed to connect")?;
