...
fn matasano3_17() -> Result<()> {
    ...
    let oracle = 
        |iv: &[u8], ciphertext: &[u8]| -> Result<bool> {
...
fn matasano3_18() -> Result<()> {
    let ciphertext = from_base64("L77na/nrFsKvynd6HzOoG7GHTLXsTVu9qvY/2syLXzhPweyyMTJULu/6/kXX0KSvoOLSFQ==")?;
    let cleartext = ciphertext.decrypt(b"YELLOW SUBMARINE", None, MODE::CTR)?;
    compare(b"Yo, VIP Let's kick it Ice, Ice, baby Ice, Ice, baby ".as_ref(), &cleartext)
}

fn matasano3_19_20(input_file: &Path) -> Result<()> {
    let cleartexts = from_base64_lines(input_file)?;
    let secret_key = random_block();
    let ciphertexts = cleartexts.iter().map(|c| c.encrypt(&secret_key, None, MODE::CTR).map_err(|err| err.into())).collect::<Result<Vec<Vec<u8>>>>()?;
    let size = ciphertexts.iter().map(|c| c.len()).min().unwrap();
    let mut transposed_blocks: Vec<Vec<u8>> = (0..size).map(|_| Vec::with_capacity(ciphertexts.len())).collect();
    for ciphertext in ciphertexts {
        for (&u, bt) in ciphertext[..size].iter().zip(transposed_blocks.iter_mut()) {
            bt.push(u);
        }
    }
    let key = transposed_blocks.iter().map(|b| decrypt_single_xor(b)).collect::<Vec<u8>>();
    ...
    compare(&vec![0; size].encrypt(&secret_key, None, MODE::CTR)?[1..], &key[1..]) // encrypt or decrypt?
}

fn matasano3_21() -> Result<()> {
    let mt = MersenneTwister::initialize(1);
    compare(vec![1_791_095_845, 4_282_876_139, 3_093_770_124, 4_005_303_368, 491_263, 550_290_313, 1_298_508_491, 4_290_846_341, 630_311_759, 1_013_994_432], 
...
fn matasano3_22() -> Result<()> {
    ...
    compare(Some(seed as u32), mersenne::crack_seed_from_nth(mt.nth(0).unwrap(), 0, 0..(std::u16::MAX as u32)))
}
...
fn matasano3_24() -> Result<()> {
    ...
    let random_number = <NativeEndian as ByteOrder>::read_u32(&ciphertext[4*index..4*(index+1)].xor(&[b'A'; 4]));
    compare(Some(seed as u32), mersenne::crack_seed_from_nth(random_number, index, 0..(std::u16::MAX as u32)))
}
